    ...
    Mutex::Autolock lock(m_lock);
    {
        ...
        if (plugIn == NULL) {
            ...
            if (libHandle == NULL) {
                ...
                if (libHandle == NULL) {
                    CLOGE3(cameraId, "Can't load %s library(%d, %d) %s",
                            g_plugInList[index].plugInLibName, index, pipeId, dlerror());
                    return INVALID_OPERATION;
                }
            }
            ...
            *(reinterpret_cast<void **>(&createSymbol)) = dlsym(libHandle, PLUGIN_SYMBOL_NAME);
            if (createSymbol == NULL) {
                CLOGE3(cameraId, "Can't find symbol(%s) from library(%s, %d, %d) %s",
                        PLUGIN_SYMBOL_NAME,
                        g_plugInList[index].plugInLibName, index, pipeId, dlerror());
                dlclose(libHandle);
                return INVALID_OPERATION;
            ...
            if (plugIn != NULL) {
                ...
                case PLUGIN_SCENARIO_HIFI_REPROCESSING:
                    converter = (new ExynosCameraPlugInConverterHifi(cameraId, pipeId));
                    break;
#endif
#ifdef USES_COMBINE_PLUGIN
                case PLUGIN_SCENARIO_COMBINE_PREVIEW:
                    converter = (new ExynosCameraPlugInConverterCombinePreview(cameraId, pipeId));
                    break;
                case PLUGIN_SCENARIO_COMBINE_REPROCESSING:
#ifdef USE_DUAL_CAMERA
                case PLUGIN_SCENARIO_COMBINEFUSION_REPROCESSING:
#endif
                    converter = (new ExynosCameraPlugInConverterCombineReprocessing(cameraId, pipeId));
                    break;
#endif
                ...
                case PIPE_NFD_REPROCESSING:
                    converter = (new ExynosCameraPlugInConverterVPL(cameraId, pipeId));
                    break;
#endif
#ifdef USES_CAMERA_EXYNOS_LEC
                case PIPE_PLUGIN_LEC_REPROCESSING:
                    converter = (new ExynosCameraPlugInConverterLEC(cameraId, pipeId));
                    break;
#endif
                default:
    ...
    Mutex::Autolock lock(m_lock);
    {
        ...
        if (!refCount) {
            ...
            if (libHandle != NULL) {
                ...
                if (ret) {
                    CLOGE3(cameraId, "dlclose is failed!!(%d) %s", pipeId, dlerror());
                    ret = INVALID_OPERATION;
                }
            }
