...
#include "mpi_lls_interface.h"
#include "sensor_type.h"

namespace android {

/*
 * Class ExynosCameraPlugInHiFiLLS
 *
...
class ExynosCameraPlugInHiFiLLS : public ExynosCameraPlugIn {
public:
    ExynosCameraPlugInHiFiLLS() : ExynosCameraPlugIn() {}
    ExynosCameraPlugInHiFiLLS(int cameraId, int pipeId, int mode) : ExynosCameraPlugIn(cameraId, pipeId, mode) {
        strncpy(m_name, "HiFiLLSPlugIn", (PLUGIN_NAME_STR_SIZE - 1));
        mUser = NULL;
        mNumFaces = 0;
    ...
    virtual ~ExynosCameraPlugInHiFiLLS() { ALOGD("%s", __FUNCTION__); };
