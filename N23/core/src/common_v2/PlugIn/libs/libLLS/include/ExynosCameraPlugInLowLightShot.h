...
#ifndef EXYNOS_CAMERA_PLUGIN_LOW_LIGHT_SHOT_H
#define EXYNOS_CAMERA_PLUGIN_LOW_LIGHT_SHOT_H
...
#include "LowLightShot.h" /* external lib header */

namespace android {

/*
 * Class ExynosCameraPlugInLowLightShot
 *
 * This is adjusted "Tempate method pattern"
 */
class ExynosCameraPlugInLowLightShot : public ExynosCameraPlugIn {
public:
    ExynosCameraPlugInLowLightShot() : ExynosCameraPlugIn() {}
    ExynosCameraPlugInLowLightShot(int cameraId, int pipeId, int mode) : ExynosCameraPlugIn(cameraId, pipeId, mode) {
        strncpy(m_name, "LowLightShotPlugIn", (PLUGIN_NAME_STR_SIZE - 1));
    };
    virtual ~ExynosCameraPlugInLowLightShot() { ALOGD("%s", __FUNCTION__); };
...
#endif //EXYNOS_CAMERA_PLUGIN_LOW_LIGHT_SHOT_H
