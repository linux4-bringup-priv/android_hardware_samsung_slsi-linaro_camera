...
public:
     ...
     *   # setprop vendor.hal.camera.debug.trap.enable true
     *   # setprop vendor.hal.camera.debug.trap.words "fatal error|device error"
     *      //User can assign multiple string to property by delimeter "|"
     *   # setprop vendor.hal.camera.debug.trap.event "panic"
     *      //panic : kernel panic
     *      //assert : process kill
     *   # setprop vendor.hal.camera.debug.trap.count 5
     *      //after decreasing this number to 0, trap would be triggered
     *
     *  3. Change the assert action
     *   # setprop vendor.hal.camera.assert "errlog"
     ...
     *  <Property>
     *  1. Path
     *   # setprop vendor.hal.camera.log.perframe.enable true
     *   # setprop vendor.hal.camera.log.perframe.path.enable true
     *
     *  2. Size
     *   # setprop vendor.hal.camera.log.perframe.enable true
     *   # setprop vendor.hal.camera.log.perframe.size.enable true
     *
     *  3. Buf
     *   # setprop vendor.hal.camera.log.perframe.enable true
     *   # setprop vendor.hal.camera.log.perframe.buf.enable true
     *   # setprop vendor.hal.camera.log.perframe.buf.filter "MCSC_OUT_BUF|DEPTH_MAP_BUF"
     *
     *  4. Meta
     *   * Only the field that user set in "filter" property can be only printed.
     *   # setprop vendor.hal.camera.log.perframe.enable true
     *   # setprop vendor.hal.camera.log.perframe.meta.enable true
     *   # setprop vendor.hal.camera.log.perframe.meta.filter "ctl.aa.afMode|ctl.aa.aeTargetFpsRange|ctl.aa.vendor_isoMode"
     *     //print ctl.aa.afMode, ctl.aa.aeTargetFpsRange and ctl.aa.vendor_isoMode
     *   # setprop vendor.hal.camera.log.perframe.meta.values "1|15,30|100"
     ...
     *
     *  5. Filtering
     *   # setprop vendor.hal.camera.log.perframe.filter.cameraId "0|1"
     *     //print the log if cameraId is 0 or 1
     *   # setprop vendor.hal.camera.log.perframe.filter.factory "reproc|jpeg_reproc"
     ...
     *  <Property>
     *  1. Fps
     *   # setprop vendor.hal.camera.log.performance.enable true
     *   # setprop vendor.hal.camera.log.performance.fps.enable true
     *   # setprop vendor.hal.camera.log.performance.frames 120
...
private:
    ...
    static std::map<std::string, std::string> kMetaChangeMap;
};
