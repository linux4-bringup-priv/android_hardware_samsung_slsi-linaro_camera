...
public:
    ...
    status_t increaseBufferRefCnt(ExynosCameraBuffer buffer);
    status_t getBuffer(const buffer_manager_tag_t tag, ExynosCameraBuffer *buffer);
    status_t putBuffer(ExynosCameraBuffer buffer);
    status_t getBatchBufferIndex(const buffer_manager_tag_t tag, int &index);
    status_t putBatchBufferIndex(ExynosCameraBuffer buffer);
    int getNumOfAvailableBuffer(const buffer_manager_tag_t tag);
    char *getName(const buffer_manager_tag_t tag);
    void dump(int fd = -1);
    void dump(const buffer_manager_tag_t tag);
